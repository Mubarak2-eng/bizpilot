    ...
    <div className="min-h-screen bg-mesh-dark text-slate-900 dark:text-white flex flex-col justify-between selection:bg-indigo-500/30 selection:text-indigo-900 dark:selection:text-indigo-200 antialiased relative overflow-hidden">
      ...
      <main className="max-w-4xl mx-auto px-6 py-20 text-center space-y-8 flex-1 flex flex-col justify-center items-center relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-12 text-left w-full">
          <div className="p-5 bg-white/90 dark:bg-[#090e24]/70 border border-slate-200 dark:border-white/[0.08] rounded-2xl backdrop-blur-xl space-y-1.5 shadow-md">
            <div className="w-7 h-7 rounded-lg bg-violet-500/15 border border-violet-500/30 flex items-center justify-center text-xs text-violet-700 dark:text-violet-300 font-bold mb-2">
              🛡️
            </div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white">Multi-Tenant Isolation</h2>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Strict tenant data isolation and role-based permissions (OWNER, ADMIN, STAFF).</p>
          </div>

          <div className="p-5 bg-white/90 dark:bg-[#090e24]/70 border border-slate-200 dark:border-white/[0.08] rounded-2xl backdrop-blur-xl space-y-1.5 shadow-md">
            <div className="w-7 h-7 rounded-lg bg-cyan-500/15 border border-cyan-500/30 flex items-center justify-center text-xs text-cyan-700 dark:text-cyan-300 font-bold mb-2">
              🧠
            </div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white">BizPilot Autopilot</h2>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Real-time telemetry, daily action plans, and automated financial write previews.</p>
          </div>

          <div className="p-5 bg-white/90 dark:bg-[#090e24]/70 border border-slate-200 dark:border-white/[0.08] rounded-2xl backdrop-blur-xl space-y-1.5 shadow-md">
            <div className="w-7 h-7 rounded-lg bg-emerald-500/15 border border-emerald-500/30 flex items-center justify-center text-xs text-emerald-700 dark:text-emerald-300 font-bold mb-2">
              💬
            </div>
            <h2 className="text-sm font-bold text-slate-900 dark:text-white">WhatsApp AI Bot</h2>
            <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">Interactive WhatsApp bot for instant voice/text sales logging and invoice dispatch.</p>
          </div>
        </div>
      </main>
