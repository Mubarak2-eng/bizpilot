  ...
  return (
    <div className="w-full max-w-md p-7 sm:p-9 bg-[#090e24]/85 border border-violet-500/30 rounded-3xl shadow-[0_0_50px_-10px_rgba(139,92,246,0.3)] backdrop-blur-2xl relative overflow-hidden">
      ...
      <div className="text-center mb-7 relative z-10">
        ...
        <h1 className="text-2xl font-black text-white tracking-tight">Sign in to BizPilot AI</h1>
        <p className="text-xs text-slate-400 mt-1">Autonomous Business Operating System</p>
      </div>

      {registered && (
        <div className="mb-5 p-3.5 rounded-xl bg-emerald-950/30 border border-emerald-500/30 text-emerald-300 text-xs font-medium relative z-10">
      ...
      <form action={formAction} className="space-y-4 relative z-10">
        ...
        >
          {isPending ? "Authenticating Session..." : "Sign In to Business OS"}
        </button>
      </form>

      {/* Quick Demo Credentials Switcher */}
      <div className="mt-7 pt-5 border-t border-white/[0.08] relative z-10">
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          <button
            ...
            className="px-2.5 py-2 bg-white/[0.04] hover:bg-emerald-500/15 border border-white/[0.08] hover:border-emerald-500/30 rounded-xl transition text-center cursor-pointer group"
          >
            <span className="font-bold text-[10px] text-emerald-400 block tracking-wider">OWNER</span>
            <span className="text-[10px] text-slate-400 group-hover:text-white font-mono truncate block">demo@...</span>
          </button>
          <button
            type="button"
            onClick={() => handleQuickFill("admin@bizpilot.test")}
            className="px-2.5 py-2 bg-white/[0.04] hover:bg-amber-500/15 border border-white/[0.08] hover:border-amber-500/30 rounded-xl transition text-center cursor-pointer group"
          >
            <span className="font-bold text-[10px] text-amber-400 block tracking-wider">ADMIN</span>
            <span className="text-[10px] text-slate-400 group-hover:text-white font-mono truncate block">admin@...</span>
          </button>
          <button
            type="button"
            onClick={() => handleQuickFill("staff@bizpilot.test")}
            className="px-2.5 py-2 bg-white/[0.04] hover:bg-blue-500/15 border border-white/[0.08] hover:border-blue-500/30 rounded-xl transition text-center cursor-pointer group"
          >
            <span className="font-bold text-[10px] text-blue-400 block tracking-wider">STAFF</span>
            <span className="text-[10px] text-slate-400 group-hover:text-white font-mono truncate block">staff@...</span>
          </button>
        </div>
      </div>

      <div className="mt-5 text-center text-xs text-slate-400 relative z-10">
        Don&apos;t have an account?{" "}
        <Link href="/signup" className="text-cyan-400 hover:text-cyan-300 font-semibold underline">
