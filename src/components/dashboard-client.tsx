...
export default function DashboardClient({
  ...
  return (
    <div className="space-y-6">
      {/* Top Bar Header */}
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 bg-[#090e24]/80 border border-white/[0.08] rounded-3xl backdrop-blur-xl shadow-xl">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-violet-600 via-indigo-600 to-cyan-400 p-[1.5px] shadow-[0_0_20px_rgba(99,102,241,0.4)]">
            <div className="w-full h-full bg-[#080c1d] rounded-[14px] flex items-center justify-center font-black text-white text-base tracking-tight">
              BP
            </div>
          </div>
          <div>
            ...
            <p className="text-xs text-slate-400 font-mono">
              Slug: <code className="text-cyan-300">{activeBusiness.slug}</code> | Currency: <span className="text-slate-300 font-medium">{activeBusiness.currency}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          {memberships.length > 1 && (
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                disabled={isPending}
                className="px-3 py-1.5 bg-[#050816] border border-white/[0.1] rounded-xl text-white text-xs focus:ring-1 focus:ring-violet-500 focus:outline-none"
              >
                {memberships.map((m) => (
                  <option key={m.id} value={m.id} className="bg-[#090e24] text-white">
                    {m.name} ({m.role})
                  </option>
                ))}
              </select>
          ...
          <button
            ...
            className="px-3.5 py-2 bg-white/[0.05] hover:bg-rose-500/20 text-slate-200 hover:text-rose-300 rounded-xl border border-white/[0.08] hover:border-rose-500/30 text-xs font-medium transition cursor-pointer"
          >
            Sign Out
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="p-6 bg-[#090e24]/70 border border-white/[0.08] rounded-3xl space-y-4 backdrop-blur-xl">
          <h2 className="text-xs font-bold text-white uppercase tracking-wider">Tenant Context</h2>
          <div className="space-y-2 text-xs">
            ...
            <div className="flex justify-between py-1.5 border-b border-white/[0.04]">
              ...
              <span className="text-cyan-300 font-medium font-mono">{memberships.length} Business{memberships.length > 1 ? "es" : ""}</span>
            </div>
        ...
        <div className="p-6 bg-[#090e24]/70 border border-white/[0.08] rounded-3xl space-y-4 backdrop-blur-xl">
          <h2 className="text-xs font-bold text-white uppercase tracking-wider">Security Architecture</h2>
        ...
        <div className="p-6 bg-[#090e24]/70 border border-white/[0.08] rounded-3xl space-y-4 backdrop-blur-xl">
          <h2 className="text-xs font-bold text-white uppercase tracking-wider">Role Action Verifier</h2>
          ...
          <div className="space-y-2.5">
            <button
              ...
              disabled={isPending}
              className="w-full py-2.5 px-3 bg-amber-500/15 hover:bg-amber-500/25 text-amber-300 border border-amber-500/30 rounded-xl text-xs font-medium transition cursor-pointer disabled:opacity-50 text-left flex justify-between items-center"
            >
            ...
            <button
              ...
              disabled={isPending}
              className="w-full py-2.5 px-3 bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-300 border border-emerald-500/30 rounded-xl text-xs font-medium transition cursor-pointer disabled:opacity-50 text-left flex justify-between items-center"
            >
